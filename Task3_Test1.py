def QuantizationTest1(file_name, Your_EncodedValues, Your_QuantizedValues):
    expectedEncodedValues = []  # Store the expected binary-encoded values
    ...
    with open(file_name, 'r') as f:
        ...
        line = f.readline()
        while line:
            # Process each line from the expected output
            L = line.strip()
            if len(L.split(' ')) == 2:
                L = L.split(' ')
                expectedEncodedValues.append(L[0])  # Store the expected binary-encoded value
